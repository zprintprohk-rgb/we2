...
export function FileUploadZone({
  ...
  }, [files.length, maxFiles, accept, maxSize, t, onUploadError])
  ...
  const uploadFile = async (uploadFile: UploadFile) => {
    ...
    try {
      // 1. 获取上传 URL（预签名或直接上传）
      const metadataRes = await fetch('/api/files/upload', {
        method: 'POST',
      ...
      const { url, key } = await metadataRes.json()

      // 2. 使用 XMLHttpRequest 追踪进度
      await new Promise<void>((resolve, reject) => {
        ...
        xhr.open('PUT', url, true) // 如果 R2 支持 PUT 直传；否则改为 POST 到 /api/files/upload 的第二步
      ...
      const completedFiles = files
        ...
        .map((f) => ({ url: f.url!, key: f.id, name: f.name }))
